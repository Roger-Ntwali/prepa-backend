...
// Admin-only. Accepts a real multipart PDF upload (field name: file) --
// the same upload.single('file') pattern already used by
// /questions/import-pdf, rather than the two routes each doing their own
// thing. A plain file_url string is still accepted for an admin who
// already has the file placed on the server directly; the upload takes
// priority when both are somehow present.
async function createPastPaper(req, res) {
  ...
  try {
    const resolvedFileUrl = req.file
      ? await saveUploadedPdf(req.file, { title })
      : (file_url || null);
    ...
    const { rows } = await pool.query(
      ...
      [title, year, term || null, topic_id || null, resolvedFileUrl, req.user.id]
    );
    res.status(201).json({ past_paper: rows[0] });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Failed to register past paper' });
  }
}
...
async function deletePastPaper(req, res) {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: 'Failed to remove past paper' });
  }
}
